...
{    
    ...
    geofence_breach_happened = false;
    global_position_status_available = false;

    currentStatus.timestamp = "";
    currentStatus.battery_percentage = 0.0;
    currentStatus.geofence_status = {0, 0, 0, 0, 0};
    currentStatus.gps_satellites = 0;
    currentStatus.gps_status = "No Fix";
    currentStatus.autopilot_mode = "Not Available";
    currentStatus.current_waypoint = "No Waypoint";
    currentStatus.global_position = {0, 0, 0, 0, 0, 0, 0, 0, 0};
    currentStatus.diagnostics = "Not Available";
    currentStatus.copter_command = "No Command";
}
...
void AFS_Gateway::step(const radl_in_t* i, const radl_in_flags_t* i_f, radl_out_t* o, radl_out_flags_t* o_f)
{
    try {
        ...
        if (this->battery_status_mailbox) {
            ...
            currentStatus.battery_percentage = o->battery_status->remaining_percentage
        ...
        if (this->geofence_status_available) {
            ...
            currentStatus.geofence_status = o->geofence_status;
            if (o->geofence_status->breach_status == 1)
            {
                geofence_breach_happened = true;
            }
        ...
        cout << "AFS Gateway Status at " << formatTimestamp(current_time) << "\n"
             << "-----------------------------------\n"
             << "Battery Remaining: " << currentStatus.battery_percentage << "%\n"
             << "Autopilot Mode: " << currentStatus.autopilot_mode << "\n"
             << "Copter Command: " << currentStatus.copter_command << "\n";
             << "Current Waypoint: " << currentStatus.current_waypoint << "\n"
             << "Global Position: " << currentStatus.global_position << "\n"
             << "GPS: " << currentStatus.gps_status << " (Satellites: " << currentStatus.gps_satellites << ")\n"             
             << "Geofence Breach: Status = " << breach_status[currentStatus.geofence_status.breach_status] << "\n";

        if (currentStatus.geofence_status.breach_status == 1)
        {
            cout << "        # Breaches = " << (int) currentStatus.geofence_status.breach_count
                 << ", Breach Type = " << breach_types[currentStatus.geofence_status.breach_type] << "\n"
                 << "        Breach Time = " << (int) currentStatus.geofence_status.breach_time << "ms (since boot of last breach)"  << "\n"
                 << "DEBUGGING ONLY: Breach Mitigation (none/velocity limiting/position limiting/landing/return to launch): " << (int) currentStatus.geofence_status.breach_mitigation << "\n"
        }
             << "Diagnostics: " << currentStatus.diagnostics << "\n"
